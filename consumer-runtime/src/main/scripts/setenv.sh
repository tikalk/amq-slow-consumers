...
consumer_home=$(cd `dirname $0`/..; pwd)
consumer_conf=${consumer_home}/conf
...
# ------------------------------------------------------------------
# set the properties in $consumer_conf/$1.properties as system properties
# the sed command will replace the placeholder in the properties file ${consumer_home} with the value of
# consumer_home, and the property consumer.home is then injected to the jvm as -D property.
# the second sed command will remove comment lines starting with #.
# the last sed command removes empty lines.
# ------------------------------------------------------------------
JAVAOPTS=`cat ${consumer_conf}/$1.properties | sed s:'${consumer_home}':${consumer_home}:g  | sed '/^\#/d' | sed '/^$/d'`
...
CLASSPATH=${consumer_conf}
for f in $consumer_home/lib/*.jar; do
